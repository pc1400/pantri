...
app.post('/scan', async (req, res) => {
  ...
  var dataToSend;
  const python = spawn('python', ['/Users/patrickcunningham/Programming/recipe/recipe/server/test.py']);
  console.log("HYs");
  python.stdout.on('data', async function (data) {
    ...
    const user = await User.findById('6379434e41121f9226ba1e13');
    var userIngredients = user.ingredients;
    dataToSend.forEach(function(x){
      if (x != "") {
        userIngredients.push(x.toLocaleLowerCase());
      }
    });
    User.updateOne(
      { _id: ObjectId('6379434e41121f9226ba1e13') },
...
app.post('/export', async (req, res) => {
  ...
  userGroceries.forEach(r => {
    if (!userIngredients.includes(r)) {
      userIngredients.push(r);
    }
  });

  User.updateOne(
          { _id: ObjectId('6379434e41121f9226ba1e13') },
          { $set: { ingredients: userIngredients, groceries: [] } }
  ).then(result => {
    updateRecipes();
    }).catch(err => {
      console.log(err);
